...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!COLOR_REGEX.test(color)) {
            message.channel.send("Incorrect color, sir ! (#XXXXXX or #XXX hex colors, type color picker in google)");
            return;
        }

        if (color.length === 4) {
            const colorArray = color.slice(1, 4).split("");
            color = "#" + colorArray[0] + colorArray[0] + colorArray[1] + colorArray[1] + colorArray[2] + colorArray[2];
        }

        if (!client.colors[message.guild.id]) {
            console.log(`New entry for ${message.guild.name} [${message.guild.id}] to colors file.`);
            client.colors[message.guild.id] = {};
        }

        if (!client.colors[message.guild.id][message.author.id]) {
            console.log(`New member for ${message.guild.name} : ${message.author.username}`);
            client.colors[message.guild.id][message.author.id] = {}
        } else if (client.colors[message.guild.id][message.author.id].cooldown){
            const remainingTime = client.colors[message.guild.id][message.author.id].cooldown + BASE_CD - Date.now();
            if (remainingTime > 0) {
        ...
        const roles = message.member.roles.cache.filter(
        ...
        roles.forEach((v, k) => {
            if (v.hexColor.toLowerCase() === color.toLowerCase()) {
                ...
                message.member.roles.remove(v)
                    .catch((reson) => "Error while deleting color role.");
            }
        });

        if (already_wear) { return }

        let guildRole;

        message.guild.roles.cache.forEach((v, k) => {
            if (v.hexColor === color) { guildRole = v };
        });
        ...
        if (!guildRole) {
            ...
            console.log(`Created one new color role for ${message.guild.name} with ${color} color.`);
        }

        message.member.roles.add(guildRole)
            .catch((reason) => console.log("Error while adding color role"));
